...
try:
    q = c.cursor()
    q.execute(sql)
    for test in ('colunas_atividade.sql', 'clientes_arquivado.sql', 'client_save.sql', 'pastas_atividade.sql', 'team_permissions.sql', 'compartilhamentos.sql'):
        phase = test
    ...
    c.commit()
    print('PASTAS_ORIGEM_MIGRATION_APPLIED')
