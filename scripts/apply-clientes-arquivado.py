...
version = '20260911030000'
name = 'clientes_arquivado'
...
try:
    q = c.cursor()
    q.execute(sql)
    for test in ('clientes_arquivado.sql', 'client_save.sql', 'pastas_atividade.sql', 'team_permissions.sql'):
        phase = test
    ...
    c.commit()
    print('CLIENTES_ARQUIVADO_APPLIED')
