...
try:
    ...
    cursor.execute("SELECT schemaname, tablename FROM pg_tables WHERE schemaname IN ('public', 'auth', 'storage') ORDER BY 1, 2")
    tables = cursor.fetchall()
    for schema, table in tables:
        cursor.execute('SELECT count(*) FROM "' + schema + '"."' + table + '"')
        print(schema + '.' + table + ': ' + str(cursor.fetchone()[0]))
    cursor.execute("SELECT extname FROM pg_extension ORDER BY 1")
    print('EXTENSIONS: ' + ', '.join(row[0] for row in cursor.fetchall()))
    connection.rollback()
